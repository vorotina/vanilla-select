...
c.query+'" class="select__query">');return b+'\n                    <ul ref="list" class="select__list">\n                        '+c.dataset.map(function(a,b){var d=a.content||a.text||a.value;return'\n                                <li class="select__item '+(a===c.selected||a.value===c.selected?"select__item--selected":"")+" "+(a.class||"")+'" data-index="'+b+'" data-value="'+d+'">\n                                    <i class="select__item_icon '+(a.icon||"")+'" aria-hidden="true"></i>\n                                    <span class="select__item_text">'+
d+"</span>\n                                </li>"}).join("")+"\n                    </ul>"};e.prototype.componentDidUpdate=function(){var a=this,c=this.$el,d=c.getBoundingClientRect();d.top+d.height>window.innerHeight&&(c.style.marginTop=-c.clientHeight-c.parentElement.clientHeight+"px");if(this.props.search){var e=this.refs.query;e.addEventListener("change",this.onQueryChanged);this.$defer("updated",function(){return e.removeEventListener("change",a.onQueryChanged)})}var f=this.refs.list;f&&(f.addEventListener("click",
this.onItemClicked),this.$defer("updated",function(){return f.removeEventListener("click",a.onItemClicked)}))};return d});
